...
const ImageControl = (props) => {
  ...
  return (
    <div className="skt-blocks-image-control">
        ...
        <MediaUpload
          ...
          render={(obj) => {
            return (
              <Fragment>
                {props.imageURL && (
                  <div className="skt-blocks-image-preview-wrapper">
                    <button
                      className="skt-blocks-image-preview-remove"
                      onClick={onRemove}
                    >
                      <Dashicon icon="no" />
                    </button>
                    {type === "video" && (
                      <video
                        className="skt-blocks-image-preview"
                        autoPlay
                        muted
                        loop
                        src={props.imageURL}
                ...
                {!props.imageURL && (
                  <div
                    className="skt-blocks-placeholder"
                    onClick={obj.open}
                    onKeyDown={(event) => {
                      if (event.keyCode === 13) {
                        obj.open();
